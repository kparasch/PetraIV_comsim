...
for bpm_id, magnet_id in zip(knobs.bpms_no_disp, knobs.bba_quads):
    if SC.RING[bpm_id].FamName in ['BPM_02', 'BPM_08']:
        bpm_ids.append(bpm_id)
        bpm_quads.append(magnet_id)
bpm_ids = np.tile(bpm_ids, (2, 1))
bpm_quads = np.tile(bpm_quads, (2, 1))
quad_is_skew = False
...
SC.orbits = []
SC.bps = []
SC, bba_offsets_skew, bba_offset_errors_skew = orbit_bba(SC, bpm_ids, bpm_quads, quad_is_skew=quad_is_skew,
                                                         n_k1_steps=n_k1_steps, max_dk1=max_dk1,# max_x=max_x, 
                                                         n_k2_steps=n_k2_steps, max_dk2=max_dk2, 
                                                         RM=ORM, plot_results=True)


pSC._save_and_check_repr(SC.RING, f'after_bpm_02_08_BBA_seed{seed}.repr')
